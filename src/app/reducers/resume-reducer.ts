...
import { 
  RESUME_ADD_AWARDS,
    ...
    RESUME_ADD_INTEREST, 
    RESUME_ADD_LANGUAGE, 
    RESUME_ADD_OBJECTIVE, 
    RESUME_ADD_PROJECT_DETAIL, 
    RESUME_ADD_REFERENCE, 
    RESUME_ADD_SKILL, 
    RESUME_ADD_STRENGTH, 
    RESUME_ADD_SUCCESS, 
    RESUME_ADD_WEAKNESS, 
    ...
    RESUME_DELETE_OBJECTIVE, 
    RESUME_DELETE_PROJECT_DETAIL, 
    RESUME_DELETE_REFERENCE, 
    RESUME_DELETE_SKILL, 
    RESUME_DELETE_STRENGTH, 
    RESUME_DELETE_WEAKNESS, 
    RESUME_ERROR, 
    RESUME_LIST_REQUEST, 
    RESUME_LIST_SUCCESS, 
    RESUME_UPDATE, 
    ...
    RESUME_UPDATE_OBJECTIVE, 
    RESUME_UPDATE_PROJECT_DETAIL, 
    RESUME_UPDATE_REFERENCE, 
    RESUME_UPDATE_SKILL, 
    RESUME_UPDATE_STRENGTH,
    RESUME_UPDATE_WEAKNESS
...
export function ResumeReducer(state = initialState, action: Action): ResumeReducerState {
  switch (action.type) {
    ...
    case RESUME_LIST_SUCCESS: {
      ...
      const ids = resume.map((res: { _id: any; }) => res._id);
    ...
    case RESUME_DELETE_SKILL: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.skills = oldResume.skills.filter((data: { _id: any; }) => data._id !== skill._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_DELETE_EDUCATION: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.education = oldResume.education.filter((data: { _id: any; }) => data._id !== education._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_DELETE_EMPLOYMENT_HISTORY: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.employment_history = oldResume.employment_history.filter((data: { _id: any; }) => data._id !== employmentHistory._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_DELETE_INTEREST: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.interests = oldResume.interests.filter((data: { _id: any; }) => data._id !== interest._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_DELETE_LANGUAGE: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.languages = oldResume.languages.filter((data: { _id: any; }) => data._id !== language._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_DELETE_INDUSTRIAL_EXPOSURE: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.industrialExposures = oldResume.industrialExposures.filter((data: { _id: any; }) => data._id !== industrialExposure._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case RESUME_UPDATE_AWARDS: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.award_achivements = oldResume.award_achivements.filter((data: { _id: any; }) => data._id !== award._id);
      oldResume.award_achivements.push(award);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    ...
    case RESUME_UPDATE_OBJECTIVE: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.objectives = oldResume.objectives.filter((data: { _id: any; }) => data._id !== objective._id);
      oldResume.objectives.push(objective);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    ...
    case RESUME_UPDATE_REFERENCE: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.refrences = oldResume.refrences.filter((data: { _id: any; }) => data._id !== reference._id);
      oldResume.refrences.push(reference);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    ...
    case RESUME_UPDATE_PROJECT_DETAIL: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.projectDetails = oldResume.projectDetails.filter((data: { _id: any; }) => data._id !== projectDetail._id);
      oldResume.projectDetails.push(projectDetail);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    ...
    case RESUME_UPDATE_STRENGTH: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.strengths = oldResume.strengths.filter((data: { _id: any; }) => data._id !== strength._id);
      oldResume.strengths.push(strength);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    ...
    case RESUME_UPDATE_WEAKNESS: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.weakness = oldResume.weakness.filter((data: { _id: any; }) => data._id !== weakness._id);
      oldResume.weakness.push(weakness);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
